...
| V01-V07 | VLM: image chat, streaming SSE, stream-vs-sync parity, usage accounting, slot recovery after a disconnect (no abort API, so the generation finishes), video input (every frame counted: two more frames add the same prompt tokens each time, however many frames the spec packs into a step), and the vision budget guard's 400 — **config-gated**; V07 additionally needs the server started with `VLM_VISION_BUDGET_GUARD` on |
