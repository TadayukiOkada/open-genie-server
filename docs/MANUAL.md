...
- Each request's `model` field (in the body, or the `?model=` query param) decides **which slot it's routed to** (`SlotManager.select`). If no slot matches, it always falls back to the primary slot, so `lm_eval`'s fixed `"genie-local"` string and single-slot deployments never need any client-side changes. Any other name that matches nothing is logged once at WARNING, so a typo does not go unnoticed.
