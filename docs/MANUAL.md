...
| `VLM_MAX_IMAGE_PIXELS` / `VLM_MAX_TOTAL_PIXELS` | optional | `16777216` / `1073741824` | Largest image or frame, and largest total across one request's images and frames, in pixels; over either is a `400`. `0` = no limit. Images are decoded at full size and all held at once (3 bytes per pixel) before the spec resizes each, and a flat-colour PNG of 13000 × 13000 (just under Pillow's own ceiling) is about 500 KB on the wire and 483 MB decoded — so the body limit does not bound this. Checked from the headers, before decoding. Defaults are 4096 × 4096, above every image the test suites and probes send, and 64 times that, 64 being the most images any of them sends in one request — so a test within the per-image limit is never stopped by the total. That total is 3 GB decoded; set it lower for a board with less to spare. They bound the server's own memory, not what reaches the SDK: a request past the context still reaches it unless `VLM_VISION_BUDGET_GUARD` is on. |
...
| Input | text | text, `image_url` and `video_url` (one encoder step per frame, `mm:ss` timestamps) |
...
| Image resolution | — | one patch grid per slot (`vision-param`); every image and video frame is resized to it |
