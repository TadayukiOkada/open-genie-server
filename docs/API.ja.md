...
- `GET /ready` と `GET /v1/ready` は readiness プローブ。VLM スロットを含む
  全スロットがモデルを持っていれば `200` `{"status": "ready", "slots":
  [{"name", "loaded"}, ...]}`、持っていないスロットがあれば `503` で
  `"status": "not ready"` と `not_loaded` の一覧を返す。モデル切り替えの最中と、
  `unload_first` の切り替えに失敗してスロットが空になった後がこれにあたり、
  後者は `/health` では見えない。エラーのエンベロープではなく状態の本文。
...
- リクエストの本文は、`Content-Type: application/json`(`application/*+json` も可)で送る
  ...
  ブラウザが事前の問い合わせなしに別オリジンへ送れる本文だからです。`MAX_REQUEST_BODY_MB`
  (既定 64 MB)を超える本文は `413`。CORS ヘッダは、呼び出し元のオリジンが
  `CORS_ALLOW_ORIGINS` に載っているときだけ返します。
...
| `model` | string | どのスロットに送るか選択(`SlotManager.select`)。一致するスロットが無ければプライマリスロット(`genie-local` 以外の名前は WARNING で 1 回だけログに出るので、typo を見つけられる。画像付きリクエストが最初の VLM スロットに回る場合も同様)。**レスポンスにはこの文字列をechoしません** — 応答本体もストリーミングチャンクも、**実際に応答したモデルのID**(選択されたスロットが現在保持しているもの)を返します。エイリアス(`genie-local`、lm_evalの固定プレースホルダ等)で振り分けた場合と、ホットスワップ後に `env_config.json` の記載と異なるモデルが載っている場合に、両者は食い違います。 |
...
`video_url`はクライアント側で抽出済みのフレーム列を運びます(`video/jpeg`メディアタイプの下にbase64 JPEGをカンマで連結した、vLLMがクライアント側前処理に使う形式)。フレームのタイムスタンプの元になる`fps`/`frames_indices`は`media_io_kwargs.video`(ボディ直下。OpenAIクライアントからは`extra_body`)で渡します。タイムスタンプは`qwen3_vl`スロットでは`<t seconds>`マーカー、`gemma4`スロットでは`mm:ss`で、`fps`が無ければどちらも書きません。この経路に固有のリクエストエラーは6つで、いずれも`400`です:

- **`video/jpeg`以外のメディアタイプ**。本サーバにデマルチプレクサは無いので、コンテナ(`data:video/mp4;base64,...`)は中途半端に扱わず拒否します。
- **リモートの`http(s)` URL**。画像と同じく取りに行きません。
- **フレームが1つも入っていない`video_url`**。
- **base64 ではないペイロード**。改行は無視し、それ以外の base64 の文字でないものは拒否します(Python の既定のデコーダのように読み飛ばしません)。
- **`VLM_MAX_IMAGE_PIXELS` を超える画像・フレーム、または画像とフレームの合計が `VLM_MAX_TOTAL_PIXELS` を超えるリクエスト**(既定は 4096 × 4096 と、その 64 倍。1 枚の上限に収まる画像なら 64 枚まで必ず通ります)。どちらも画像のヘッダから判定し、デコードの前に断ります。エンコーダはどれも入力を数百画素四方に縮めるので、クライアント側で縮めても失うものはありません。`image_url` にも適用します。
- **スロットのコンテキストに収まらない量の視覚入力**。`VLM_VISION_BUDGET_GUARD`がONのときだけで、既定はOFFです。メッセージは何ステップ入るのかとその理由を示します。OFF(既定)のときに何が起きるかは[視覚入力の上限](./MANUAL.ja.md#視覚入力の上限)を参照 — 綺麗には落ちません。
...
- 往復の履歴も理解します: `tool_calls` 付きassistantメッセージは `<tool_call>` ブロックに再描画され、`role: "tool"` の結果メッセージは `<tool_response>` ブロック(chatml)/ `ipython` ターン(llama3)として、モデル自身のチャットテンプレート通りに描画されます。gemma4 スロットではどちらもその方言で再描画されます — 呼び出しは `<\|tool_call>call:NAME{...}<tool_call\|>`、結果は `<\|tool_response>response:NAME{...}<tool_response\|>` を載せた user ターンです。NAME は tool メッセージの `name`、無ければその `tool_call_id` と一致する、それ以前の assistant の `tool_calls[].id` から取ります。どちらも無い結果は `400` です。`arguments` が JSON のオブジェクトでない呼び出しも、gemma4 の記法では書き戻せないので `400` です(Hermes はモデルが出した生のテキストのまま残します)。空文字列の `arguments` は引数なしの呼び出しとして扱います。
...
2. **無ければ `model`** — 各スロットがロードしているモデルと突き合わせます。
   **誰もロードしていない名前は、失敗ではなくプライマリスロットに落ちます** —
   `lm_eval` が全リクエストに固定のプレースホルダを送るためです。`genie-local` 以外の
   そうした名前は、ロード中のモデルの一覧とともに WARNING で 1 回だけログに出るので、
   フォールバックに乗った typo を見つけられます。画像付きリクエストが最初の VLM スロットに
   回る場合も同じです(ロード中のテキストモデルの名前なら出しません。画像は名前に関係なく
   VLM スロットに回るためです)。
...
パラメータ検証・存在しないスロット・SDK失敗・不正なJSONボディ・内部の`HTTPException`・サーバが想定していない例外まで、**全ての**エラーがOpenAIのエラーエンベロープで返るため、OpenAI SDK / lm_eval / LiteLLMは常に失敗をパースできます:
...
| `415` | リクエストの本文が `Content-Type: application/json` で送られていない |
| `422` | 意味的に処理不能(llama2テンプレートでのprefix warmup要求など) |
| `500` | SDK呼び出し失敗、モデルロード失敗、またはサーバが想定していない例外。最後のものはバグで、メッセージには例外の型名と `err-1a2b3c4d` のようなエラー ID だけが入り、traceback はその ID でサーバのログに残る |
