...
- **`VLM_MAX_IMAGE_PIXELS` を超える画像・フレーム、または画像とフレームの合計が `VLM_MAX_TOTAL_PIXELS` を超えるリクエスト**(既定は 4096 × 4096 と、1920 × 1080 で約 64 フレーム)。どちらも画像のヘッダから判定し、デコードの前に断ります。エンコーダはどれも入力を数百画素四方に縮めるので、クライアント側で縮めても失うものはありません。`image_url` にも適用します。
