...
| `temperature` / `top_p` / `top_k` | number | Sampling parameters, re-applied per request: a request that omits one gets the **model's own `genie_config.json` default** back (no leakage from the previous request's settings). `temperature=0` means greedy decoding (implemented as `top-k=1` — the SDK's runtime sampler-config path cannot take a literal temp of 0). Ranges: `temperature` ≥ 0, `top_p` 0–1, `top_k` an integer ≥ 0 (0 = no top-k limit; vLLM's `-1`, which means the same, is accepted and sent as 0, since the SDK reads top-k as unsigned). Anything else, a string included, is a `400`. |
| `seed` | int | Best-effort sampling seed, forwarded to the SDK sampler. Omitted, the request gets a fresh random seed, so earlier requests' seeds never carry over. A `seed` in the bundle's sampler config only seeds the dialog once, when it is created. For reproducible output, send `seed`. An integer from 0 to 2³¹−1. |
| `logprobs` | int (0-20) | Returns per-token logprobs for the **generated** tokens, plus that many top alternatives per position (see [Logprobs](./MANUAL.md#logprobs)). Non-streaming only. Combined with `echo`, switches to **prompt scoring** (lm_eval loglikelihood) — gated behind `POST /v1/server/prompt_logprobs`. A QAIRT runtime that never calls the logits callback gets HTTP 400 `logprobs_not_supported`. |
...
- The tool round-trip history is understood on the way back in: assistant messages with `tool_calls` are re-rendered as `<tool_call>` blocks, and `role: "tool"` result messages are rendered as `<tool_response>` blocks (chatml) / `ipython` turns (llama3), matching the models' own chat templates. A gemma4 slot re-renders both in its own dialect — the call as `<\|tool_call>call:NAME{...}<tool_call\|>`, the result as a user turn holding `<\|tool_response>response:NAME{...}<tool_response\|>`. NAME comes from the tool message's `name`, or else from the earlier assistant `tool_calls[].id` its `tool_call_id` matches; a result with neither is a `400`. So is a call whose `arguments` are not a JSON object, which gemma4's notation cannot write back (Hermes keeps the raw text the model emitted); an empty `arguments` string is a call with no arguments.
- **llama2 and Gemma 2/3 slots refuse tool history with a `400`.** Their chat templates have no form for a tool call or a tool result; llama2 used to drop tool results outright, and both dropped the calls, so the model saw the round trip with pieces missing. A llama2 slot likewise refuses a role other than `system`, `user` and `assistant` rather than dropping it.
...
**Every** error — parameter validation, unknown slots, SDK failures, even malformed JSON bodies, internal `HTTPException`s and exceptions the server did not expect — is returned in the OpenAI error envelope, so OpenAI SDKs / lm_eval / LiteLLM can always parse failures:
