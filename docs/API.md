...
| `seed` | int | Best-effort sampling seed, forwarded to the SDK sampler. Omitted, the request gets a fresh random seed, so earlier requests' seeds never carry over. A `seed` in the bundle's sampler config only seeds the dialog once, when it is created. For reproducible output, send `seed`. |
