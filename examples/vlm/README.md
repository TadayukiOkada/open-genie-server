...
Send a picture of ordinary size. The encoder resizes it to a few hundred pixels square anyway, and one over `VLM_MAX_IMAGE_PIXELS` (4096 × 4096 by default) is refused with a `400` before it is decoded. A photo straight off a recent phone camera, at 48 MP, is over the limit: scale it down first, or raise the setting.
