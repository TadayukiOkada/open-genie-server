...
画像は普通の大きさで送ってください。エンコーダはどのみち一辺数百画素に縮めます。`VLM_MAX_IMAGE_PIXELS`(既定 4096 × 4096)を超える画像は、デコードの前に `400` で断ります。最近のスマートフォンのカメラで撮ったままの 48 MP の写真は上限を超えるので、先に縮めるか設定を上げてください。
